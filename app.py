...
def h4(svg, text, size=18, bg="#e8f4fd", color="#1a73e8"):
    return f'<h4 style="display:flex;align-items:center;margin:10px 0 6px 0;">{icon(svg,size,bg,color)}{text}</h4>'
...
ICO_PAW    = '<path d="M10.5 2a8.5 8.5 0 1 0 0 17 8.5 8.5 0 0 0 0-17z"/><path d="M21 21l-4.35-4.35"/>'
...
# ══════════════════════════════════════════════════════════
    ...
    with col1:
        ...
        st.markdown("""Build a computer vision system that can **automatically identify the animal species** in a given image.
        The system classifies images into **15 distinct animal categories**, useful for wildlife monitoring, education, and ecological research.""")
    ...
    with col_a:
        st.markdown("- **Wildlife Conservation:** Automating species identification from camera traps\n- **Education:** Interactive tool for learning about animals")
    with col_b:
        st.markdown("- **Biodiversity Monitoring:** Rapid species census from image databases\n- **Veterinary Applications:** Assisting in animal identification")
...
# ══════════════════════════════════════════════════════════
    ...
    with c1: st.markdown('<div class="metric-card"><h2>~92%</h2><p>Accuracy</p></div>', unsafe_allow_html=True)
    with c2: st.markdown('<div class="metric-card"><h2>~91%</h2><p>Precision</p></div>', unsafe_allow_html=True)
    with c3: st.markdown('<div class="metric-card"><h2>~92%</h2><p>Recall</p></div>', unsafe_allow_html=True)
    with c4: st.markdown('<div class="metric-card"><h2>~91%</h2><p>F1-Score</p></div>', unsafe_allow_html=True)

    st.markdown("<br>", unsafe_allow_html=True)
    col1, col2 = st.columns(2)
    with col1:
        st.markdown(h3(ICO_CPU, "Model Architecture"), unsafe_allow_html=True)
        st.markdown("""| Component | Details |\n|---|---|\n| Base | CNN with Transfer Learning |\n| Input Shape | 224 × 224 × 3 |\n| Output Classes | 15 |\n| Activation (Final) | Softmax |\n| Loss Function | Categorical Cross-Entropy |\n| Optimizer | Adam |\n| Preprocessing | Pixel Normalization [0, 1] |""")
        st.markdown(h3(ICO_LAYERS, "Why CNN & Transfer Learning?"), unsafe_allow_html=True)
        st.markdown("- **CNNs** are state-of-the-art for image classification tasks.\n- **Transfer Learning** leverages a model pre-trained on millions of images (ImageNet).\n- The model extracts hierarchical features: edges → textures → animal parts → full species identity.")
    with col2:
        st.markdown(h3(ICO_CHART, "Class Distribution"), unsafe_allow_html=True)
        fig = go.Figure(go.Bar(x=CLASS_NAMES, y=[200]*15, marker_color='#4fc3f7', text=[200]*15, textposition='outside'))
        fig.update_layout(xaxis_title="Animal", yaxis_title="Images", height=300,
            margin=dict(t=10,b=60), plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)', xaxis_tickangle=-45)
        st.plotly_chart(fig, use_container_width=True)

    with st.expander("Preprocessing Pipeline"):
        st.code("""img = image.resize((224, 224))
img_array = np.array(img) / 255.0
img_array = np.expand_dims(img_array, axis=0)  # (1, 224, 224, 3)""", language="python")
